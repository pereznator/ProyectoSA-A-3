...
const updateTipoMetodoPago = async (req, res) => {
  const { id } = req.params;
      ...
      if (response_search.result.length > 0) {
        query_format
          .queryFormatWithTransaction(
            `
          UPDATE proyecto.tipo_metodo_pago
        ...
        return res.status(200).json({ log: "TIPO DE METODO DE PAGO NO ENCONTRADO" });
      }
    })
    .catch((error) => {
      console.log(error);
