...
const deleteColaborador = async (req, res) => {
  ...
  query_format
    .queryFormatWithTransactionArray([
      {
        query: `select convert(cognito_sub, char) as cognito_sub from usuario where id = (select usuario_id from colaborador where id = ?)`,
        inserts: [id],
      },
      {
        query: `DELETE FROM usuario
        WHERE usuario.id=(select usuario_id from colaborador where colaborador.id = ?)`,
        inserts: [id],
      },
    ])
